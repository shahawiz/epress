...
$search_element = $_GET['search'];
$today_date=date("l, Fd, Y");

/* Search By Category */
$search_category_content = $db->query("select p.post_image,p.post_id,p.post_title,p.date,p.content,c.cat_name from posts p inner join categories c on p.cat_id=c.cat_id where c.cat_name='{$search_element}' ");
$show_search_category_content = $search_category_content->fetchAll();

/* Search By Title */
$search_title_content = $db->query("select p.post_image,p.post_id,p.post_title,p.date,p.content,c.cat_name from posts p inner join categories c on p.cat_id=c.cat_id where p.post_title like '%{$search_element}%' ");
$show_search_title_content = $search_title_content->fetchAll();

/* Search By Journalist */
$search_name_content = $db->query("select p.post_image,p.post_id,p.post_title,p.date,p.content,u.username from posts p inner join users u on p.owner_id=u.user_id where u.username ='{$search_element}' ");
$show_search_name_content = $search_name_content->fetchAll();
...
/* Technology Article */
$tech_content = $db->query("select p.post_id,p.content,p.post_title,p.date,p.post_image from posts p inner join categories c on c.cat_id=p.cat_id where cat_name='technology'");
$show_tech_content = $tech_content->fetchAll();
$tech_new_date = date("Fd, Y",strtotime($show_tech_content[0]['date']));

/* Sport Article */
$sport_content = $db->query("select p.post_id,p.content,p.post_title,p.date,p.post_image from posts p inner join categories c on c.cat_id=p.cat_id where cat_name='sport'");
$show_sport_content = $sport_content->fetchAll();
$sport_new_date = date("Fd, Y",strtotime($show_sport_content[0]['date']));

/* Economy Article */
$economy_content = $db->query("select p.post_id,p.content,p.post_title,p.date,p.post_image from posts p inner join categories c on c.cat_id=p.cat_id where cat_name='economy'");
$show_economy_content = $economy_content->fetchAll();
$economy_new_date = date("Fd, Y",strtotime($show_economy_content[0]['date']));

/* Entertainment Article */
$entertain_content = $db->query("select p.post_id,p.content,p.post_title,p.date,p.post_image from posts p inner join categories c on c.cat_id=p.cat_id where cat_name='entertainment'");
$show_entertain_content = $entertain_content->fetchAll();
$entertain_new_date = date("Fd, Y",strtotime($show_entertain_content[0]['date']));

/* politic Article */
$politic_content = $db->query("select p.post_id,p.content,p.post_title,p.date,p.post_image from posts p inner join categories c on c.cat_id=p.cat_id where cat_name='politic'");
$show_politic_content = $politic_content->fetchAll();
$politic_new_date = date("Fd, Y",strtotime($show_politic_content[0]['date']));

/* culture Article */
$culture_content = $db->query("select p.post_id,p.content,p.post_title,p.date,p.post_image from posts p inner join categories c on c.cat_id=p.cat_id where cat_name='culture'");
$show_culture_content = $culture_content->fetchAll();
$culture_new_date = date("Fd, Y",strtotime($show_culture_content[0]['date']));

/* travelling Article */
$travel_content = $db->query("select p.post_id,p.content,p.post_title,p.date,p.post_image from posts p inner join categories c on c.cat_id=p.cat_id where cat_name='travelling'");
	...
	<div class="main-content container">
		<div class="col-md-8 blog-single">
			...
			<div class="search-results row">
				<div class="col-md-12">
					<h3>Search result for : <span><?php echo $search_element?></span></h3>
				</div>


<?php
foreach($show_search_category_content as $value){
			echo "
				<div class='col-md-12'>
					<div class='fn2-inner'>
						<div class='fn2-thumb'>
							<img src='images/{$value['post_image']}' class='img-responsive' alt=''/>
						</div>
						<div class='fn2-info'>
							<div class='fn2-meta'>{$search_element}</div>
							<h4><a href='article.php?postId={$value["post_id"]}'>{$value['post_title']}</a></h4>
							<em>Posted on {$value['date']}</em>
							<p>{$value['content']}</p>
						</div>
					</div>
				</div>
			...
			foreach($show_search_title_content as $value){
				echo "
					<div class='col-md-12'>
						<div class='fn2-inner'>
							<div class='fn2-thumb'>
...
foreach($show_search_name_content as $value){
	echo "
		<div class='col-md-12'>
			<div class='fn2-inner'>
				<div class='fn2-thumb'>
...
?>		
					...
					<ul>
						...
						<li>
							...
							<div class="pn-info">
								...
								<h4><a href="article.php?postId=<?php echo $show_tech_content[0]['post_id']?>"><?php echo $show_tech_content[0]['post_title']?></a></h4>
							</div>
						</li>
					</ul>
				</div>
	...
	<footer class="container">
		<div class="col-md-4 footer-widget footer-logo">
			...
			<span class="copy">Copyright &copy; 2019 Gazeta. Web Design by Naguib</span>
		</div>
